...
toggleBtn.addEventListener("click", () => {
    ...
    headerStack.classList.toggle("active");
...
if (typeof Splide !== "undefined" && document.querySelector(".logo-showcase-slider.splide")) {
    new Splide(".logo-showcase-slider", {
        ...
        breakpoints: { 991: { perPage: 5 }, 767: { perPage: 4 }, 375: { perPage: 3 } },
    }).mount(window.splide.Extensions);
}
if (typeof Splide !== "undefined" && document.querySelector(".clients-site-showcase.splide")) {
    const splide = new Splide(".clients-site-showcase.splide", {
        autoplay: !0,
        rewind: !0,
        gap: "28px",
        width: "auto",
        ...
        speed: 800,
        easing: "ease-in-out",
        breakpoints: { 991: { gap: "24px" }, 767: { gap: "20px" }, 575: { gap: "12px" } },
    });
    splide.mount();
}
...
if (processFlow) {
    const wrapper = document.querySelector(".process-flow-wrapper");
    const contentList = document.querySelector(".process-flow-content-list");
    ...
    function resetProcessFlowHeight() {
        if (!wrapper) return;
    ...
    function setProcessFlowHeight() {

        if (!wrapper || !contentList) return;


        /*
          Mobile / Tablet
    
          Remove dynamic height completely.
        */
        ...
        /*
          Calculate the biggest required height
        */

        const maxHeight = getMaxProcessFlowHeight();
        ...
        if (maxHeight > 0) {
            ...
            /*
              IMPORTANT:
              Use height, not min-height.
      
              This locks the section height
              while tabs are changing.
            */
    ...
    function activateItem(index) {
        ...
        */
        const effect = items[index].querySelector(
            ".process-flow-time-line-effect"
        );
        if (effect) {
    ...
    function startDesktopTabs() {
        ...
        /*
          First calculate and lock
          the section height.
        */
        setProcessFlowHeight();
        /*
          Then activate tab
        */
        activateItem(currentIndex);
    ...
    function stopDesktopTabs() {
        ...
        /*
          ...
          Mobile/tablet becomes natural height.
        */
        resetProcessFlowHeight();
    }
    /* ========================================
...
======================================== */
    ...
    window.addEventListener("resize", () => {
        ...
        resizeTimer = setTimeout(() => {
            ...
            // Only recalculate when width changes
...
if (typeof Splide !== "undefined" && document.querySelector(".partners-showcase-slider")) {
    new Splide(".partners-showcase-slider", {
        ...
        pagination: !1,
        autoScroll: { speed: 0.7 },
        breakpoints: { 991: { perPage: 4 }, 575: { perPage: 3 } },
    }).mount(window.splide.Extensions);
}
if (typeof Splide !== "undefined" && document.querySelector(".highlight-slider")) {
    document.addEventListener("DOMContentLoaded", function () {
        ...
        var main = new Splide(".highlight-slider", { type: "fade", pagination: !1, perPage: 1 });
...
if (typeof Splide !== "undefined" && document.querySelector(".client-testimonials-slider")) {
    document.addEventListener("DOMContentLoaded", function () {
        ...
        var main = new Splide(".client-testimonials-slider", { type: "fade", pagination: !1, perPage: 1 });
        main.sync(thumbnails);
        main.mount();
        thumbnails.mount();
    });
...
const BOX_STYLES = [
    { fill: "#FF333705", border: "#FF33370A", largeInset: "#FF333729", topInset: "#FF333729", edgeInset: "#FF33371F" },
    { fill: "#FF33370A", border: "#FF33371F", largeInset: "#FF333729", topInset: "#FF333729", edgeInset: "#FF33371F" },
    { fill: "#FF33371F", border: "#FF33373D", largeInset: "#FF333729", topInset: "#FF333729", edgeInset: "#FF33371F" },
];
    ...
    function findBoxPosition(ignoredBox = null) {
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < columns; x++) {
                const overlapsExistingBox = boxes.some((box) => box !== ignoredBox && box.x === x && box.y === y);
...
document.querySelectorAll(".process-hero-animation").forEach((section) => {
    ...
    function drawBoxes() {
        boxes.forEach((box) => {
            ...
            context.fillStyle = style.fill.replace(/([\d.]+)\)$/, (_, alpha) => `${Number(alpha) * opacity})`);
            context.fillRect(x, y, boxSize, boxSize);
            if (intensity > 0) {
                context.fillStyle = `rgba(255,71,74,${intensity * opacity * 0.16})`;
                context.fillRect(x, y, boxSize, boxSize);
            }
            context.strokeStyle = style.border.replace(/([\d.]+)\)$/, (_, alpha) => `${Number(alpha) * opacity})`);
...
if (typeof Splide !== "undefined" && document.querySelector(".blog-card-slider")) {
    const splide = new Splide(".blog-card-slider.splide", {
        perPage: 3,
        gap: "20px",
        pagination: !1,
        breakpoints: { 991: { perPage: 2, gap: "18px" } },
    });
    splide.mount();
}
...
if (tabs.length && cards.length && searchInput && loadMoreBtn) {
    ...
    function filterBlogs() {
        ...
        let matchedCards = [];
        cards.forEach((card) => {
            const category = card.querySelector(".articles-card-caption").textContent.trim().toLowerCase();
...
if (typeof Splide !== "undefined" && document.querySelector(".case-study-hero-slider.splide")) {
    document.addEventListener("DOMContentLoaded", function () {
        var splide = new Splide(".case-study-hero-slider.splide", {
            ...
            easing: "ease-in-out",
            breakpoints: {
                991: { gap: "20px", heightRatio: 1.56 },
                767: { gap: "12px", direction: "ltr", heightRatio: 0, focus: "left" },
            },
        });
        splide.mount();
    });
...
if (typeof Splide !== "undefined" && document.querySelector(".featured-slider.splide")) {
    document.addEventListener("DOMContentLoaded", function () {
        var main = new Splide(".featured-slider.splide", { type: "fade", perPage: 1, pagination: !1 });
